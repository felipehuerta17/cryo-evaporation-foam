...
def eta_u_fobj(x):
    ...
    with open("tl.log", "w") as outfile:
        subprocess.run(
            ["postProcess", "-func", "singleGraph", "-time", "2400"], stdout=outfile
        )
    ...
    # Compute vapour temperature residual

    # Assign T(t=2400) as the optimal
    temp_v_exp = T_exp[1]
    temp_v_res = np.interp(z_exp, z, temp_mod) - temp_v_exp
    ...
    # Compute liquid temperature residual
    temp_l_res = np.interp(zl_exp, zl_cfd, tl_cfd) - tl_exp[1]

    RMSE_tl = np.sqrt(np.mean((np.interp(zl_exp, zl_cfd, tl_cfd) - tl_exp[1]) ** 2))
    print("Liquid temperature RMSE = %.3f K " % RMSE_tl)

    L = [p_res, temp_v_res, temp_l_res]
    loss = (
    ...
    print("Loss = %.3f, eta = %.3f, U = %.3f" % (loss, x[0], x[1]))
